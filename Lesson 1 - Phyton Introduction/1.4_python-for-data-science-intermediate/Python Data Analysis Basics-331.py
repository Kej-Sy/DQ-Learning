...
string = template.format(name = artist, year = birth_year)
print(string)
...
def artist_summary(artist):
    n_of_works = artist_freq[artist]
    template = "There are {arts} artworks by {name} in the dataset"
    string = template.format(arts = n_of_works, name = artist)
    print(string)
...
for row in pop_millions:
    country_name = row[0]
    n_of_population = row[1]
    string = template.format(country = country_name, population = n_of_population)
...
for gender, artworks in gender_freq.items():
    template = "There are{number_of_arts: ,} artworks by {artists_gender} artists"
    string = template.format(number_of_arts = artworks, artists_gender = gender)
    print(string)