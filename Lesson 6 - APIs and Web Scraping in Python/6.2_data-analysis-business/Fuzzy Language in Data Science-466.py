...
cutoff_day = pd.datetime(2019, 9, 16)
...
transactions = best_churn["nr_of_transactions"]

best_churn["scaled_tran"] = (transactions - transactions.min()) / (transactions.max() - transactions.min())

amount = best_churn["amount_spent"]

best_churn["scaled_amount"] = (amount - amount.min()) / (amount.max() - amount.min())
